...
echo "Updating cmac_jessica.py..."
# Using perl instead of sed for better macOS compatibility
perl -i -pe "s|url=f\"https://.*?/outbound-call-handler\"|url=f\"$NGROK_URL/outbound-call-handler\"|g" cmac_jessica.py

echo "Updating recording callback URL..."
perl -i -pe "s|recording_status_callback=\"https://.*?\"|recording_status_callback=\"$NGROK_URL/recording-status-callback\"|g" cmac_jessica.py
